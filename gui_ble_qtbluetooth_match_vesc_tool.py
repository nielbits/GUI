...
# Use Qt Bluetooth to mirror VESC Tool BLE behavior as closely as possible.
try:
...
# =========================
# Qt BLE transport (modeled after VESC Tool)
# =========================
class BleScanWorker(QtCore.QObject):
    ...
    @QtCore.pyqtSlot()
    def run(self):
        if QtBluetooth is None:
            self.failed.emit("QtBluetooth is not available in this PyQt5 build")
            return

        try:
            self.agent = QtBluetooth.QBluetoothDeviceDiscoveryAgent()
            self.agent.setLowEnergyDiscoveryTimeout(4000)
            ...
            def add_device(dev):
                try:
                    cfg = int(dev.coreConfigurations())
                    is_le = bool(cfg & int(QtBluetooth.QBluetoothDeviceInfo.LowEnergyCoreConfiguration))
                except Exception:
                    is_le = True
            ...
            def fail(_err):
                try:
                    err_str = self.agent.errorString()
                    if not err_str:
                        err_str = "BLE scan failed"
                    self.failed.emit(err_str)
                finally:
                    loop.quit()

            self.agent.deviceDiscovered.connect(add_device)
            self.agent.finished.connect(finish)
            try:
                self.agent.error.connect(fail)
            except Exception:
                self.agent.errorOccurred.connect(fail)

            self.agent.start(QtBluetooth.QBluetoothDeviceDiscoveryAgent.LowEnergyMethod)
            loop.exec_()
...
class BleUartTransport(QtCore.QObject):
    """
    QtBluetooth BLE transport modeled after VESC Tool bleuart.cpp:
      - discover LE device
      - connect with QLowEnergyController
      - discover services
      - create NUS service object
      - discover details
      - enable TX notifications via CCC descriptor
      - write RX with WriteWithoutResponse
      - split writes into 20-byte chunks
    Exposes serial-like methods for pyvesc.
    """
    connected_changed = QtCore.pyqtSignal(bool)

    def __init__(self, device_info, timeout=0.25, parent=None):
        super().__init__(parent)
        if QtBluetooth is None:
            raise RuntimeError("QtBluetooth is not available in this PyQt5 build")
        ...
        self._service = None
        self._notification_desc_tx = None
        self._uart_service_found = False
    ...
    @staticmethod
    def _device_addr(dev):
        if sys.platform.startswith("darwin"):
            return dev.deviceUuid().toString()
        return dev.address().toString()

    def _worker_thread_main(self):
        self._loop = QtCore.QEventLoop()
        self._worker = QtCore.QObject()
    ...
    def open(self):
        ...
        def do_connect():
            self._start_connect_qt()

        self._run_in_qt(do_connect)

        deadline = time.perf_counter() + (BLE_CONNECT_TIMEOUT_MS / 1000.0 + 2.0)
        while not self._connect_done and self._connect_error is None:
    ...
    def close(self):
        ...
        try:
            self._run_in_qt(self._disconnect_qt)
        except Exception:
            pass
    ...
    def _start_connect_qt(self):
        ...
        if sys.platform.startswith("darwin"):
            self._controller = QtBluetooth.QLowEnergyController.createCentral(self.device_info)
        else:
            self._controller = QtBluetooth.QLowEnergyController.createCentral(self.device_info)

        try:
            self._controller.setRemoteAddressType(QtBluetooth.QLowEnergyController.RandomAddress)
        except Exception:
            pass

        self._controller.serviceDiscovered.connect(self._service_discovered)
        self._controller.discoveryFinished.connect(self._service_scan_done)

        try:
            self._controller.error.connect(self._controller_error)
        except Exception:
            self._controller.errorOccurred.connect(self._controller_error)

        self._controller.connected.connect(self._device_connected)
        self._controller.disconnected.connect(self._device_disconnected)

        try:
            self._controller.stateChanged.connect(self._control_state_changed)
        except Exception:
            pass

        try:
            self._controller.connectionUpdated.connect(self._connection_updated)
        except Exception:
            pass

        self._controller.connectToDevice()

    def _disconnect_qt(self):
    ...
    def _service_discovered(self, gatt):
        if gatt == QtBluetooth.QBluetoothUuid(QtCore.QUuid(BLE_SERVICE_UUID)):
            self._uart_service_found = True

    def _service_scan_done(self):
        if self._service is not None:
            try:
                self._service.deleteLater()
            except Exception:
        ...
        if not self._uart_service_found:
            self._connect_error = "BLE UART service not found"
            self._disconnect_qt()
            return

        self._service = self._controller.createServiceObject(
            QtBluetooth.QBluetoothUuid(QtCore.QUuid(BLE_SERVICE_UUID)), None
        )

        if self._service is None:
            self._connect_error = "Failed to create BLE UART service object"
            self._disconnect_qt()
            return

        self._service.stateChanged.connect(self._service_state_changed)
        try:
            self._service.error.connect(self._service_error)
        except Exception:
            self._service.errorOccurred.connect(self._service_error)

        self._service.characteristicChanged.connect(self._update_data)
        self._service.descriptorWritten.connect(self._confirmed_descriptor_write)
        self._service.discoverDetails()

    def _controller_error(self, _e):
        try:
            err = self._controller.errorString()
        except Exception:
            err = "BLE controller error"
        self._connect_error = f"BLE error: {err}"
        self._disconnect_qt()
    ...
    def _device_connected(self):
        try:
            ...
            self._connect_error = f"BLE discoverServices failed: {e}"
            self._disconnect_qt()

    def _device_disconnected(self):
        self._disconnect_qt()

    def _service_state_changed(self, s):
        if s == QtBluetooth.QLowEnergyService.InvalidService:
            self._disconnect_qt()
            return

        if s == QtBluetooth.QLowEnergyService.ServiceDiscovered:
            tx_char = self._service.characteristic(QtBluetooth.QBluetoothUuid(QtCore.QUuid(BLE_TX_UUID)))
            rx_char = self._service.characteristic(QtBluetooth.QBluetoothUuid(QtCore.QUuid(BLE_RX_UUID)))

            if not tx_char.isValid():
                self._connect_error = "BLE TX characteristic not found"
                self._disconnect_qt()
                return

            if not rx_char.isValid():
                self._connect_error = "BLE RX characteristic not found"
                self._disconnect_qt()
                return

            self._notification_desc_tx = tx_char.descriptor(QtBluetooth.QBluetoothUuid.ClientCharacteristicConfiguration)
            if self._notification_desc_tx.isValid():
                self._service.writeDescriptor(self._notification_desc_tx, QtCore.QByteArray.fromHex(b"0100"))
            else:
                self._connect_error = "BLE TX notification descriptor not found"
                self._disconnect_qt()

    def _service_error(self, _e):
        pass

    def _update_data(self, c, value):
        if c.uuid() == QtBluetooth.QBluetoothUuid(QtCore.QUuid(BLE_TX_UUID)):
            b = bytes(value)
            with self._rx_cv:
                self._rx_buf.extend(b)
                self._rx_cv.notify_all()

    def _confirmed_descriptor_write(self, d, value):
        if d.isValid() and d == self._notification_desc_tx and bytes(value) == b"0000":
            self._disconnect_qt()
        else:
    ...
    def _control_state_changed(self, _state):
        pass

    def _connection_updated(self, _new_parameters):
        pass
    ...
    def write(self, data):
        ...
        def do_write():
            try:
                rx_char = self._service.characteristic(QtBluetooth.QBluetoothUuid(QtCore.QUuid(BLE_RX_UUID)))
                if not rx_char.isValid():
                    raise RuntimeError("BLE RX characteristic not valid")

                idx = 0
                while idx < len(data):
                    chunk = data[idx:idx + BLE_WRITE_CHUNK]
                    self._service.writeCharacteristic(
                        rx_char,
                        QtCore.QByteArray(bytes(chunk)),
        ...
        QtCore.QTimer.singleShot(0, do_write)
        done.wait(timeout=5.0)

        if err_box["exc"] is not None:
            raise OSError(f"BLE write failed: {err_box['exc']}")
...
def attach_custom_io(vesc):
    ...
    def request_custom(self, msg):
        # Match VESC Tool packet behavior more closely: raw single-byte command payload.
        frame = encode(bytes([msg.id]))
        with serial_lock:
            try:
                try:
                    self.serial_port.reset_input_buffer()
...
class Ui_MainWindow(object):
    ...
    def on_ble_scan_finished(self, devices):
        ...
        # Match VESC Tool style: prefer VESC-like or UART service devices.
        scored = []
        for dev in devices:
            try:
            ...
            score = 0
            if "VESC" in display_name.upper():
                score += 100
            if BLE_DEVICE_NAME_HINT.upper() in display_name.upper():
                score += 20
            ...
            try:
                ...
                if uuids:
                    for u in uuids:
                        if str(u).lower() == BLE_SERVICE_UUID.lower():
                            score += 50
                            break
            except Exception:
                pass

            scored.append((score, display_name, addr, dev))

        scored.sort(key=lambda x: (-x[0], x[1].lower(), x[2].lower()))

        for _score, display_name, addr, dev in scored:
            display_text = f"{display_name} - {addr}"
            self.comboBox_portselect.addItem(display_text, dev)

        # Auto-select the best match.
        if self.comboBox_portselect.count() > 0:
            self.comboBox_portselect.setCurrentIndex(0)
    ...
    def start_com(self):
        ...
        if dev is None:
            log_event("Connect requested without valid BLE selection")
        ...
        if sys.platform.startswith("darwin"):
            port_label = dev.deviceUuid().toString()
        else:
            port_label = dev.address().toString()
...
def vesc_communication():
    ...
    while True:
        ...
        port = selected_port
        if sys.platform.startswith("darwin"):
            port_label = port.deviceUuid().toString()
        else:
            port_label = port.address().toString()
        ...
        try:
            ...
            with BleUartTransport(device_info=port, timeout=0.25) as ble_port:
